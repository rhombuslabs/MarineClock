...
- **Pre-rendered chimes.** `res/raw/bells_1.ogg` … `bells_8.ogg`: mono OGG Vorbis.
  ...
  - v1.0.1: the script applies a −10 dB trim (`$gainDb`). This puts the chimes' loudest
    50 ms at about −24 dB (sox "RMS Pk"), the median of the Pixel's built-in notification
    sounds; the untrimmed chimes measured −14.4 dB. The user confirmed the new level by ear.
- **Launcher icon (v1.0.1).** An adaptive icon: the vector navy gradient background, plus
  bell foreground and monochrome (themed) PNG layers generated by `tools/make-icon.ps1`
  from `tools/icon/ships_bell_icon.jpg`.
